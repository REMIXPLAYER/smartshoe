...
object PerformanceTestUtils {
    ...
    private const val TEST_DURATION_MS = 60000L // 测试时长60秒
    private const val WARMUP_DURATION_MS = 5000L // 预热时长5秒
    ...
    fun runPerformanceTest(
        context: Context,
        testName: String,
        testDurationMs: Long = TEST_DURATION_MS,
    ...
    private fun analyzeTestResult(
        ...
        val dropFrameRate = if (fpsSamples.isNotEmpty()) {
            fpsSamples.count { it < 55 }.toDouble() / fpsSamples.size * 100
        } else 0.0
        ...
        val memoryGrowthRate = if (heapSamples.size >= 2) {
            ...
            if (firstHalf > 0) ((secondHalf - firstHalf) / firstHalf) * 100 * (60000.0 / duration) else 0.0
        } else 0.0
     ...
     * 模拟压力测试 - 生成大量数据
     */
    fun generateStressTestData(count: Int = 10000): List<SensorDataPoint> {
        val data = mutableListOf<SensorDataPoint>()
        val currentTime = System.currentTimeMillis()
        val timeRange = 30 * 60 * 1000L // 30分钟
    ...
    fun runRegressionTest(
        ...
        CoroutineScope(Dispatchers.Main).launch {
            ...
            results.add(runBasicFunctionalityTest())
            delay(500)
     ...
     * 基础功能测试
     */
    private fun runBasicFunctionalityTest(): RegressionTestResult {
        return try {
            // 验证基本组件初始化
            val memoryLeakDetector = MemoryLeakDetector.getInstance()
            val bluetoothResourceManager = BluetoothResourceManager.getInstance()
            val performanceMonitor = PerformanceMonitor.getInstance()
     ...
     * 内存泄漏测试
     */
    private fun runMemoryLeakTest(context: Context): RegressionTestResult {
        ...
        // 模拟一些操作
        val testData = generateStressTestData(1000)
        val circularBuffer = com.example.smartshoe.data.CircularBuffer<SensorDataPoint>(1000)
        ...
        val memoryGrowthPercent = (memoryDiff.toDouble() / memoryBefore) * 100

        val passed = memoryGrowthPercent < 10 // 内存增长小于10%视为通过
    ...
    private fun runPerformanceBaselineTest(context: Context): RegressionTestResult {
        ...
        // 测试数据操作性能
        val testData = generateStressTestData(5000)
        val circularBuffer = com.example.smartshoe.data.CircularBuffer<SensorDataPoint>(5000)
        ...
        // 性能标准：添加5000条数据应小于1秒，读取应小于500ms
        val addPerformanceOk = addDuration < 1000
        val readPerformanceOk = readDuration < 500
        ...
        return RegressionTestResult(
            ...
            details = """
                添加5000条数据: ${addDuration}ms (标准: <1000ms)
                读取5000条数据: ${readDuration}ms (标准: <500ms)
    ...
    private fun runBoundaryConditionTest(): RegressionTestResult {
        ...
        // 测试1: 空数据处理
        try {
            val emptyBuffer = com.example.smartshoe.data.CircularBuffer<SensorDataPoint>(100)
        ...
        // 测试2: 缓冲区溢出处理
        try {
            val smallBuffer = com.example.smartshoe.data.CircularBuffer<SensorDataPoint>(10)
            repeat(20) { i ->
                smallBuffer.add(SensorDataPoint(System.currentTimeMillis() + i, i, i, i))
            }
            val size = smallBuffer.size()
            tests.add("缓冲区溢出处理" to (size == 10))
        } catch (e: Exception) {
        ...
        // 测试3: 大数据集处理
        try {
            val largeBuffer = com.example.smartshoe.data.CircularBuffer<SensorDataPoint>(18000)
            val largeData = generateStressTestData(20000)
            largeData.forEach { largeBuffer.add(it) }
            tests.add("大数据集处理" to (largeBuffer.size() == 18000))
        } catch (e: Exception) {
