...
object DateTimeUtils {
    ...
    private val monthDayHourMinuteFormat by lazy { SimpleDateFormat("MM-dd HH:mm", Locale.getDefault()) }
    private val fileTimestampFormat by lazy { SimpleDateFormat("yyyyMMdd_HHmmss", Locale.getDefault()) }
    ...
    /**
     * 格式化为月-日 时:分 格式 MM-dd HH:mm
     */
    fun formatMonthDayHourMinute(timestamp: Long): String {
        return monthDayHourMinuteFormat.format(Date(timestamp))
    }

    /**
     * 格式化为文件时间戳格式 yyyyMMdd_HHmmss
     */
    fun formatFileTimestamp(timestamp: Long): String {
        return fileTimestampFormat.format(Date(timestamp))
    }
